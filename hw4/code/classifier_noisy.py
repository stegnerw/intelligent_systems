...
if __name__ == '__main__':
    ...
    csv_rows.append(['$\\eta$', str(CLASS_NOISY_ETA), 'Learning rate'])
    csv_rows.append(['$\\alpha$', str(CLASS_NOISY_ALPHA), 'Momentum'])
    csv_rows.append(['$max\\_epochs$', str(CLASS_NOISY_MAX_EPOCHS),
        'Maximum training epochs'])
    csv_rows.append(['$L$', str(CLASS_NOISY_L), 'Lower activation threshold'])
    csv_rows.append(['$H$', str(CLASS_NOISY_H), 'Upper activation threshold'])
    csv_rows.append(['$patience$', str(CLASS_NOISY_PATIENCE),
        'Patience before early stopping'])
    csv_rows.append(['$es\\_delta$', str(CLASS_NOISY_ES_DELTA),
        'Delta value for early stopping'])
    with open(str(CLASS_NOISY_PARAM_CSV), 'w') as csv_file:
